# ---------------------------------------------------------------------------
# prod: full retention, all three products, locked budget at $75.
# Differs from dev only in retention, expiry and budget - keep it that way so
# the dev/prod diff stays reviewable.
# ---------------------------------------------------------------------------
terraform {
  ...
  backend "gcs" {
    ...
    prefix = "crypto-market-platform/prod"
  }
}
